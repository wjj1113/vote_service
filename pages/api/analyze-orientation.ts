...
🎯 반드시 다음 기준을 반영하여 가장 적합한 **1명의 후보자**만 추천하세요:
1. 후보의 **정책 공약**과 위 성향 수치 간 **정량적 유사성**
2. 후보의 **가치관 및 리더십 스타일**과 사용자 **기반 가치 및 판단 기준** 간 정성적 일치
3. 사용자 관심 분야(정치, 사회문제)와 후보의 주요 아젠다 일치 여부
4. **정당명, 유명도, 개인 이미지 등은 고려하지 말고** 정책과 성향 일치도 기준으로만 판단
5. 이재명=진보 / 황교안=보수 등의 이분법적 대응을 피하고, 세부 성향 수치와 정책 기반 정밀 분석을 진행

⛔ 정당 인기, 유명세, 진영논리 기반 추천은 금지

💡 최종 판단은 사용자 성향 수치 기반의 실용적, 정책적 정합성에 따라 내리세요.
  ...
  try {
    ...
    if (isQuickSurvey) {
      ...
      const recommendationData = {
        ...
        matchingPoints: best.matchingPoints.map((point: { title: string }) => point.title), // 객체에서 title만 추출
